# Analysis of the Ideological Structure of the French National Assembly Using Graph Methods and Dimensionality Reduction: A Comparative Study of the 15th and 16th Legislatures

**Inès de Rosnay**

Executive Summary

This article presents a quantitative and comparative analysis of the political structure of the French National Assembly over two legislative periods (2017–2022 and 2022–2024). By representing parliamentary votes in the form of opinion vectors in a multidimensional space, we apply graph theory and statistical techniques to reveal coalition patterns, the central figures of each bloc, and ideological dynamics. Our results show a **major structural fragmentation** between the 15th legislature (dominated by a monolithic presidential bloc) and the 16th legislature (segmented into three distinct poles). We validate our methodological approach by comparing four similarity metrics and demonstrate the robustness of cosine similarity as a political visualization tool.
...
<!-- ### 10.1 Background in Quantitative Political Science -->
<!-- 
Our approach is situated within the tradition of **Spatial Models of Legislative Politics**:

- **Poole & Rosenthal (2007):** NOMINATE scores for the U.S. Congress. Our cosine approach can be seen as a **simplified geometric approximation** of NOMINATE, without parametric estimation.
- **Hinich & Munger (1994):** Continuous spatial theory of political preferences. We validate its applicability in the French context.
- **Hix et al. (2005):** Analysis of votes in the European Parliament, showing cross-party cleavages. Our study confirms this phenomenon at the national scale.

### 11.2 Network Graphs in Politics

- **Waugh et al. (2009):** Co-voting networks (edges based on voting agreements). Our approach enriches this with **weighting by continuous similarity**.
- **Newman (2003):** Community detection in social networks. We use Newman's spring layout to spatialize the political graph. -->

<!-- ### 11.3 Dimensional Analysis: PCA vs Alternatives

- **PCA (Jolliffe, 2002):** Classical linear approach. **Advantage:** Interpretability. **Limitation:** Linearity.
- **t-SNE (van der Maaten & Hinton, 2008):** Non-linear reduction. **Advantage:** Local structure preserved. **Limitation:** Problematic for very high-dimensional data (>1500).
- **UMAP (McInnes et al., 2018):** Modern alternative to t-SNE. Promising for future work.

### 11.4 Originality of This Study

1. **First systematic comparison** of four similarity metrics on the French case  
2. **Integration of graphs + PCA:** Few studies combine spatialization and dimensionality reduction  
3. **Detailed thematic analysis:** Reveals hidden cleavages beyond global polarization  
4. **Comprehensive official data:** No biased selection of roll calls -->
...
<!-- 1. **Bivalent representation of votes:** We normalize {for, against, abstain} → {+1, -1, 0}, losing the nuance between "strong disagreement" and "mild disagreement."  
   - *Possible mitigation:* Weight by vote margin (e.g., 47% for vs 53% against)

2. **Treating absence as zero:** Our choice to replace NaN with 0 in cosine similarity is robust but not neutral. A **mean-value imputation** scheme could be tested.

3. **Fixed k-NN:** Using a single $k$ does not adapt to local density variations. An **adaptive approach** (e.g., k-NN based on a similarity threshold) deserves investigation.

4. **No temporal correction:** Early votes in the legislature (context X) are not weighted differently from later votes (context Y). A **time-series approach** would enrich the analysis.

### 12.2 Empirical Limitations -->

<!-- 1. **Small number of legislatures:** Two historical periods suffice for descriptive comparison but limit statistical inference.

2. **Variable representativeness:** Deputies elected late (by-elections) have fewer votes.

3. **Data issues:** Some roll calls have missing or erroneous records in the NosDéputés API. -->
...
<!-- 
Hix, S., Noury, A., & Roland, G. (2005). Dimensions of politics in the European Parliament. *American Journal of Political Science*, 49(2), 213-234.
...
Waugh, A. S., Peixoto, T. P., & Preis, T. (2009). Reconciliation of markets and models for wheat prices. *PLoS ONE*, 4(6), e6524. -->
